...
<nav class="navbar navbar-inverse navbar-fixed-top">
		<div class="container-fluid">
			<div class="navbar-header">
				...
				<a class="navbar-brand active" href="#top"><span class="glyphicon glyphicon-user"></span></a>
			</div>
...
<?php
	if (isset($_SESSION['email_id'])) {
		...
		echo '<div class="container">
				<div class="media">
					<button id="edit" class="pull-right btn btn-primary"><span class="glyphicon glyphicon-pencil"></button>
						...
						<a data-toggle="tooltip" title="Click here to upload a new image"><img id="DP" data-toggle="modal" data-target="#uploadmodal" class="pull-right media-object img-circle img-responsive" width="170" height="170" src="/AMOC/IMG/avtar-'.$_SESSION['email_id'].'.jpg"></a></div>
		...
		$count=mysqli_query($connect,"SELECT COUNT(e_id) AS NO FROM exam WHERE u_id='$_SESSION[user_id]' ORDER BY time_started DESC");
			...
			{
				echo "<table class='table table-responsive table-hover'><th>Sr No.</th><th>Difficulty</th><th>Marks</th><th>Started at</th><th>Time</th>";
				for($i=1;$data=mysqli_fetch_assoc($record);$i++)
				{
					echo '<tr>
						  <td>'.$i.'</td>
						  <td>'.$data['difficulty'].'</td>
						  <td>'.$data["marks_obtained"].'/'.$data['total_marks'].'</td>
